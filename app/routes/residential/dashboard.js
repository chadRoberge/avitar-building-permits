  ...
  async model() {
    ...
    try {
      ...
      if (contractorsResponse.ok) {
        ...
        console.warn('Could not fetch contractors:', contractorsResponse.status);
      ...
      return {
        ...
        permits: {
          all: permits,
          pending: permits.filter(p => p.status === 'pending' || p.status === 'submitted'),
          current: permits.filter(p => p.status === 'approved' || p.status === 'active'),
          completed: permits.filter(p => p.status === 'completed' || p.status === 'closed')
        },
        contractors: contractors,
        summary: {
          totalPermits: permits.length,
          activePermits: permits.filter(p => ['approved', 'active'].includes(p.status)).length,
          pendingPermits: permits.filter(p => ['pending', 'submitted'].includes(p.status)).length,
      ...
      return {
        ...
        contractors: [],
        summary: { totalPermits: 0, activePermits: 0, pendingPermits: 0, totalContractors: 0 }
      };
    }
  }
}