...
export default class ResidentialPermitsNewRoute extends Route {
  async model() {
    ...
    try {
      ...
      const permitTypesResponse = await fetch(
        `${config.APP.API_HOST}/api/permit-types?municipality=${municipalityId}`,
        {
          headers: {
      ...
      if (permitTypesResponse.ok) {
        ...
        console.warn(
          'Could not fetch permit types:',
          permitTypesResponse.status,
        );
      }
