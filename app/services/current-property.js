...
export default class CurrentPropertyService extends Service {
  ...
  async initialize(userId) {
    ...
    try {
      ...
      if (savedPropertyId) {
        selectedProperty = this.userProperties.find(
          (p) => p._id === savedPropertyId,
        );
      }
      ...
      if (userResponse.ok) {
        ...
        if (userData.propertyAddress && userData.propertyAddress.street) {
          ...
          const createResponse = await fetch(
            `${config.APP.API_HOST}/api/properties`,
            {
              method: 'POST',
              headers: {
  ...
  async setPrimary(propertyId) {
    try {
      ...
      const response = await fetch(
        `${config.APP.API_HOST}/api/properties/${propertyId}/set-primary`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  get currentPropertyShort() {
    ...
    return (
      this.currentProperty.displayName || this.currentProperty.address.street
    );
  }
