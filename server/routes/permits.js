...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedTypes.includes(file.mimetype)) {
    ...
    cb(new Error('Invalid file type. Allowed: images, PDF, Word, Excel, text, ZIP'), false);
  }
};
...
router.get('/:permitId/files', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.userType === 'residential' && permit.applicant.id !== req.user.userId) {
      return res.status(403).json({ error: 'Access denied' });
    }
...
router.post('/:permitId/files/upload', authenticateToken, upload.array('files', 5), async (req, res) => {
  try {
    const { permitId } = req.params;
    ...
    for (const file of req.files) {
      try {
        ...
        await permitFile.populate('uploadedBy', 'firstName lastName email userType');
...
router.get('/:permitId/files/:fileId/download', authenticateToken, async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `inline; filename="${permitFile.originalName}"`);
...
// Delete a file
router.delete('/:permitId/files/:fileId', authenticateToken, async (req, res) => {
  try {
    const { permitId, fileId } = req.params;
    ...
    if (req.user.userType === 'residential' && permitFile.uploadedBy.toString() !== req.user.userId) {
      return res.status(403).json({ error: 'Access denied' });
    }
