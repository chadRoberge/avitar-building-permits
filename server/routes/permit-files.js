...
router.get('/:permitId/files', auth, async (req, res) => {
  try {
    ...
    if (req.user.userType === 'residential' && permit.applicant.toString() !== req.user._id.toString()) {
...
router.post('/:permitId/files/upload', auth, upload.array('files', 5), async (req, res) => {
  try {
    const { permitId } = req.params;
...
router.delete('/:permitId/files/:fileId', auth, async (req, res) => {
  try {
    ...
    if (req.user.userType === 'residential' && permitFile.uploadedBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({ error: 'Access denied' });
    }
