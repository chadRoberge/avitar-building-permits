...
const workflowStepSchema = new mongoose.Schema({
  ...
  actions: [{
    type: {
      type: String,
      enum: ['review', 'approve', 'request-info', 'schedule-inspection', 'payment', 'upload-document']
    },
    required: {
      type: Boolean,
      default: false
      ...
      case 'tiered':
        if (fee.baseField && applicationData[fee.baseField] && fee.tiers) {
          ...
          for (const tier of fee.tiers) {
            if (baseValue >= (tier.min || 0) && baseValue <= (tier.max || Infinity)) {
              if (tier.amount) {
...
permitTypeSchema.methods.getNextWorkflowStep = function(currentStep) {
  ...
  const currentIndex = this.workflow.findIndex(step => step.name === currentStep);
  if (currentIndex === -1 || currentIndex === this.workflow.length - 1) return null;
  
  return this.workflow[currentIndex + 1];
};

// Static methods
permitTypeSchema.statics.getByMunicipality = function(municipalityId, includeInactive = false) {
