...
// Index for searching
municipalitySchema.index({ name: 'text', 'address.city': 'text', 'address.zip': 1 });
...
municipalitySchema.methods.getSubscriptionLimits = function() {
  const limits = {
    ...
    professional: {
      ...
      users: 15,
      price: 4800,
      features: ['advanced_workflows', 'api_access', 'phone_support', 'custom_branding']
    },
    enterprise: {
      permits: null, // Unlimited
      users: null, // Unlimited
      price: null, // Custom pricing
      features: ['custom_workflows', 'full_api', 'dedicated_support', 'custom_integrations']
    }
  };
...
municipalitySchema.methods.isWithinLimits = function() {
  ...
  const checks = {
    permits: limits.permits ? this.usage.permits.currentPeriod <= limits.permits : true,
...
municipalitySchema.statics.searchMunicipalities = function(searchTerm, state = null) {
  const query = {
    isActive: true,
    $or: [
