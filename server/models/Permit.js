...
permitSchema.index({ 'projectAddress.street': 'text', 'projectAddress.city': 'text' });
...
permitSchema.pre('save', async function(next) {
  if (!this.permitNumber && this.municipality) {
    ...
    const municipality = await mongoose.model('Municipality').findById(this.municipality);
...
// Static methods
permitSchema.statics.getStatistics = function(municipalityId, year = new Date().getFullYear()) {
  const startOfYear = new Date(year, 0, 1);
  ...
  return this.aggregate([
    ...
    },
    {
      $group: {
        ...
        avgProcessingTime: { $avg: { $subtract: ['$approvedDate', '$submittedDate'] } },
        statusBreakdown: {
          $push: '$status'
        }
      }
